...
That blit is deliberately **synchronous**. `updateScreen()` is a CPU loop and uses no DMA,
which leaves the eDMA engine to the two ADC capture channels. `updateScreenAsync()` adds a
third channel running continuously and drops ~30% of the ADC samples — quietly, while the
frame rate still looks great. Read `docs/display-async.md` before touching this; it also
covers why double buffering hardfaults the board.
